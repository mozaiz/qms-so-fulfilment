...
| `deploy/test_persistence.py` | 9 | Data survives stop/restart/SIGKILL — **and four listeners can boot the same fresh database at once**, which is a real race on a first install and used to kill one of them silently. Uses real processes: a threaded version of this check passed against code that fails in CI every time |
